...
<aside id="sidebar"
  ...
  <nav class="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto custom-scrollbar">
    ...
    <a href="{{ route('beranda') }}" class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium text-white/60 hover:bg-white/10 hover:text-white transition-colors no-underline">
  ...
  <div class="px-3 py-4 border-t border-white/10">
    <div class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-white/10 cursor-pointer transition-colors">
      <div id="sidebarAvatar" class="w-8 h-8 bg-green-light rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0">{{ strtoupper(substr(Auth::user()->username, 0, 2)) }}</div>
      <div>
        <p id="sidebarName" class="text-sm font-semibold text-white leading-tight">{{ Auth::user()->username }}</p>
        <p id="sidebarRole" class="text-xs text-white/40 leading-tight">🌾 {{ ucfirst(Auth::user()->role) }}</p>
      </div>
    </div>
...
async function removeWatch(e, i) {
  ...
  try {
    const response = await fetch('/api/v1/pantauan/toggle', {
      method: 'POST',
...
async function updateChart() {
  ...
  // Coba ambil dari history API (7 hari terakhir, Nasional/Rata-rata)
  let chartData = [];
  try {
      const res = await fetch(`/api/v1/harga/history/${slug}?provinsi=Nasional`);
      const json = await res.json();
      if (json.status === 'success') {
          // Ambil harga 7 hari terakhir
          chartData = json.data.slice(-7).map(item => parseFloat(item.harga));
          
          // Jika data kurang dari 7, duplikasi data pertama
          while (chartData.length > 0 && chartData.length < 7) {
              chartData.unshift(chartData[0]);
          }
      }
  } catch (err) {
      console.error("Gagal load data chart:", err);
  }

  // Jika tidak ada data sama sekali, beri default 0
  ...
  chartInst = new Chart(ctx,{
    type:'line',
    data:{ labels:days, datasets:[{ data: chartData, borderColor:'#2d6a4f', backgroundColor:grad,
...
async function fetchNotifikasi() {
    try {
        const response = await fetch('/api/v1/notifikasi');
        const result = await response.json();
...
async function fetchWatchlist() {
    try {
        const response = await fetch('/api/v1/pantauan');
        const result = await response.json();
...
async function fetchHistoryData() {
    try {
        const response = await fetch('/api/v1/riwayat');
        const result = await response.json();
