  ...
  try {
    ...
    if (result.status === "success") {
      ...
      commodities = result.data.map(item => ({
        ...
        name: item.nama,
        unit: 'per kg',
        price: parseFloat(item.harga_terkini) || 0,
        change: parseFloat(item.perubahan) || 0,
        kategori: item.kategori ? (
                  item.kategori.toLowerCase().includes('pokok') ? 'pokok' : 
...
async function fetchProvinsiDB() {
  try {
    ...
    if (result.status === "success") {
      ...
      // Isi semua dropdown provinsi yang ada di halaman
      ['searchProvince', 'prediksiWilayah', 'inflasiProvinsi'].forEach(elId => {
...
async function fetchBeritaDB() {
  try {
    ...
    if (json.status === 'success' && json.data && json.data.length > 0) {
      // Map format Laravel API ke format yang dibutuhkan renderBerita()
      newsData = json.data.map(item => ({
        id: item.id,
        title: item.judul,
        body: item.deskripsi || item.isi || '',
        cat: item.kategori || item.slug_komoditas || 'Berita',
        date: item.tanggal || item.tanggal_publikasi || '',
        image: item.cover_image || item.gambar_url || '/img/placeholder.jpg',
        source: item.sumber || 'Redaksi',
  ...
  list.innerHTML = paginated.map(c => `
    <div class="commodity-card ${c.id===activeCommodityId?'active':''}" onclick="selectCommodity('${c.id}')">
      ...
      <div class="c-right">
        ...
        <a href="/komoditas/${c.id}" onclick="event.stopPropagation()"
...
async function updateSearchKabKota() {
  ...
  if (!provinsi || provinsi === '') {
    kabEl.disabled = true;
    kabEl.innerHTML = '<option value="">Pilih Provinsi terlebih dahulu</option>';
    return;
  }
  ...
  try {
    ...
    if (json.kab_kota && json.kab_kota.length > 0) {
      ...
      kabEl.innerHTML = '<option value="">Tidak ada kab/kota (tampil data provinsi)</option>';
    }
  } catch (err) {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      if (kabKota && kabKota !== '') {
        ...
        // Tampilkan semua wilayah (provinsi + kab/kota) yang termasuk dalam provinsi terpilih
        filteredData = filteredData.filter(r => r.wilayah === province || r.provinsi === province);
        document.getElementById('searchResultLabel').textContent = `Hasil untuk "${commodity}" di ${province} & Kab/Kota — Diperbarui hari ini`;
      } else {
        // Tidak pilih provinsi: tampilkan 6 provinsi teratas
      ...
      if (filteredData.length === 0) {
        ...
        document.getElementById('resultsGrid').innerHTML = '<p style="color:var(--text-light);padding:20px;text-align:center">Data tidak tersedia untuk wilayah ini.</p>';
      } else {
        document.getElementById('resultsGrid').innerHTML = filteredData.map(r => `
          <div class="result-card">
            <div class="result-commodity">
              ${r.tipe_wilayah === 'kab_kota' ? '<span style="font-size:.65rem;color:var(--text-light);display:block;margin-bottom:2px">Kab/Kota</span>' : ''}
              ${r.wilayah}
            </div>
    ...
    // Simpan Riwayat ke Database (hanya jika sukses, gunakan Sanctum cookie)
    if (localStorage.getItem('isLoggedIn') === 'true') {
        try {
            await fetch('/api/v1/riwayat', {
                method: 'POST',
...
async function updatePrediksiKabKota() {
  ...
  try {
    ...
    if (json.kab_kota && json.kab_kota.length > 0) {
      ...
      kabEl.innerHTML = '<option value="">Tidak ada kab/kota</option>';
    }
  } catch (err) {
...
async function renderHomePrediksiChart() {
  ...
  try {
    const res  = await fetch(`/api/v1/prediksi?slug_komoditas=${encodeURIComponent(slug)}&provinsi=${encodeURIComponent(wilayah || 'Nasional')}&days=${hari}`);
    const raw  = await res.json();
    // Normalize response: jika wrapped dalam { status, data }, unwrap-nya
    ...
    if (rekoEl) {
      ...
      if (!isLoggedIn) {
        ...
        document.getElementById('prediksiSaranText').innerHTML = "<em>Silakan <a href='/login' style='color:#2e7d32; text-decoration:underline;'>login</a> terlebih dahulu untuk mendapatkan saran tindakan.</em>";
      } else {
...
async function doLogout() {
  try {
    // Gunakan form POST logout Laravel (CSRF protected)
...
async function runInflasiPrediksi() {
  ...
  try {
    // Panggil endpoint Laravel inflasi
    const res  = await fetch(`/api/v1/inflasi?${params}`);
    const data = await res.json();
...
function renderInflasiResult(data) {
  ...
  if (rekoInflasiEl) {
    if (!isLoggedIn) {
      ...
      document.getElementById('inflasiSaranText').innerHTML = "<em>Silakan <a href='/login' style='color:#1565c0; text-decoration:underline;'>login</a> terlebih dahulu untuk mendapatkan saran tindakan.</em>";
    } else {
      let trend = 'stagnan';
      if (pct > 2) trend = 'naik';
