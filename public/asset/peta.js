...
async function openPanel(provinceName, staticData) {
  ...
  try {
    const url = `/api/v1/harga/kab-kota?provinsi=${encodeURIComponent(provinceName)}&slug=${encodeURIComponent(currentSlug)}`;
...
async function loadGeoJSON() {
  ...
  fetch('/api/v1/komoditas')
    .then(r => r.json())
    .then(d => {
      if (d.status === 'success' && Array.isArray(d.data)) document.getElementById('statKomoditas').textContent = d.data.length;
    })
    .catch(() => {
